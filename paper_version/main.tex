...
\review{
\begin{remark}
When applying the ``projection to one dimension"
  described in Section~4.1.1, the fully discrete 3D AP scheme (32) collapses to (31)
  discretization proposed in \cite{degond_2012}. This is not a coincidence, because
...
\review{
\subsection{Case II: 3D cuboid domain (convergence study for a range of $\lambda$)} 
The problem setting follows the one depicted in Fig.~\ref{fig:domain_sketch} while we adopt a cuboid domain, see Fig.~\ref{fig:cube_domain_sketch}. Under this setting, we decompose the domain with structured Cartesian meshes and benefit from simpler error computation. 
The initial conditions are
\begin{equation*}
    n_\ast(\mathbf{x}, 0) = 1, \ \ \  p_\ast(\mathbf{x},0) = 1, \ \ \forall \mathbf{x}\in\Omega, \ast \in \{e, i\}
\end{equation*}
and zero for the other variables. We set $\varphi(\mathbf{x}, t)|_{\Gamma_C^-} = 0, \varphi(\mathbf{x}, t)|_{\Gamma_C^+} = 10\times t$, that is, one electrode is grounded and a linear voltage is applied on the other electrode. The results are computed for the final time $t = 0.01$.
The scheme is tested at different mesh resolutions and the convergence rates are estimated. Tab.~\ref{tab:grid_study_cube} lists the errors for the $\mathbf{E}$-field, $\mathbf{B}$-field, and electron densities where the reference solutions are generated by a sufficiently fine mesh with $2^{18} = 262144$ primal cells. The results show uniform convergence over $0\leq \lambda \leq 1$, a justification of the AP property. It is expected that the convergence rate does not exceed one since the reconstruction of the electromagnetic field discussed in Section~\ref{sec:recon_lorentz_f} is only of first order and the numerical flux used for solving the Euler equations is also of first order. The $1/2$-order reduction observed in the $L^2$-error of the $\mathbf{E}$-field and the electron density is possibly the consequence of singularity in the solution due to the interface of different types of boundary conditions (electrodes and insulating boundary).
\begin{table}[!ht]
    \review{
    \centering
    ...
    \caption{\review{Case II:  $L^1$ and $L^2$ errors of the $\mathbf{E}$-field, the $\mathbf{B}$-field, and the electron density and their convergence rates for a range of $\lambda$. The reference solution is computed at a sufficiently mesh resolution with $2^{18} = 262144$ primal cells.}}
...
\begin{figure}
    ...
    \vspace{-2cm}
        \resizebox{\textwidth}{!}{
            \begin{tikzpicture}[>=stealth]
                % Electrode 1
                \coordinate (O1) at (0,0);
                \draw[violet,very thick] (O1) rectangle ++(2,2) node[midway, blue] {Electrode};
                \fill[blue, opacity=0.2] (O1) rectangle ++(2,2);
                
                % Electrode 2
                \coordinate (O2) at (4.5,1);
                \draw[violet,very thick] (O2) rectangle ++(2,2) node[midway, red] {Electrode};
                ...
                \draw[very thick] (_O2) rectangle ++(3,3);
    ...
    \end{minipage}
    \hspace{1cm}
    \includegraphics[scale=0.2]{./figures/cube_mesh_primal.png}
    \hspace{1cm}
    \includegraphics[scale=0.2]{./figures/cube_mesh_dual.png}
...
\review{
\subsection{Case IV: 3D bended cylindrical domain}
To demonstrate the performance of the proposed scheme on more general three-dimensional settings, we bend the cylinder domain in Case III (see Fig.~\ref{fig:domain_sketch}), as is shown in Fig.~\ref{fig:bend_domain_sketch}. The same initial and boundary conditions as Case III are used. The numerical results of the electromagnetic field for $\lambda = 0$ and $1$ are displayed in Fig.~\ref{fig:3d_vec_field_E_B_bend}. It can be observed the qualitatively similar phenomena as Case~III (see Fig.~\ref{fig:3d_vec_field_E_B_ue}), that is, the electromagnetic field in the bulk of the domain develops much faster at small $\lambda$.}
\begin{figure}
    \centering
    \begin{minipage}{0.3\textwidth}
    \vspace{-2cm}
