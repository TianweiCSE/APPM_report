...
\newcommand{\reviewDel}[1]{\begin{comment} #1 \end{comment}}
...
\review{
\subsection{Case II: 3D cuboid domain (convergence study for a range of $\epsilon$)} 
The scheme is tested at different mesh resolutions and the convergence rates are estimated. The problem setting follows the one depicted in Fig.~\ref{fig:domain_sketch} while we adopt a cuboid domain, see Fig.~\ref{fig:cube_domain_sketch}. Under this setting, we decompose the domain with structured Cartesian meshes and benefit from simpler error computation.  
The initial conditions are
\begin{equation*}
    n_\ast(\mathbf{x}, 0) = 1, \ \ \  p_\ast(\mathbf{x},0) = 1, \ \ \forall \mathbf{x}\in\Omega, \ast \in \{e, i\}
\end{equation*}
and zero for the other variables. We set $\varphi(\mathbf{x}, t)|_{\Gamma_C^-} = 0, \varphi(\mathbf{x}, t)|_{\Gamma_C^+} = 10\times t$, that is, one electrode is grounded and a linear voltage is applied on the other electrode. The results are computed for the final time $t = 0.01$. Tab.~\ref{tab:grid_study_cube} lists the errors for the $\mathbf{E}$-field, $\mathbf{B}$-field, and electron densities where the reference solutions are generated by a sufficiently fine mesh with $2^{18} = 262144$ cells. The results show uniform convergence over $0\leq \lambda \leq 1$, a justification of the AP property. It is expected that the convergence rate does not exceed one since the reconstruction of the electromagnetic field discussed in Section~\ref{sec:recon_lorentz_f} is only of first order and the numerical flux used for solving the Euler equations is also of first order. The $1/2$-order reduction observed in the $L^2$-error of the $\mathbf{E}$-field and the electron density is possibly the consequence of singularity in the solution due to the interface of different types of boundary conditions (electrodes and insulating boundary).
\begin{table}[!ht]
    \review{
    \centering
...
\begin{figure}
    ...
    \end{minipage}
    \hspace{1cm}
    \includegraphics[scale=0.2]{paper_version/cube_mesh_primal.png}
    \hspace{1cm}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \draw[blue, -to] (0,0) -- (0.7,0) node[below]{\small $y$};
        \end{tikzpicture}
        \includegraphics[scale=0.2]{E-field_lambda-1.png}\hspace{0.6cm}
        \includegraphics[scale=0.2]{B-field_lambda-1.png}
        \includegraphics[scale=0.2]{electronVel_lambda-1.png}
    ...
    \end{subfigure}
     \begin{subfigure}[b]{\textwidth}
        ...
        \draw[blue, -to] (0,0) -- (0.7,0) node[below]{\small $y$};
        \end{tikzpicture}
        \includegraphics[scale=0.2]{E-field_lambda-1e-1.png}
        \hspace{0.3cm}
        \includegraphics[scale=0.2]{B-field_lambda-1e-1.png}
        \includegraphics[scale=0.2]{electronVel_lambda-1e-1.png}
...
To demonstrate the performance of the proposed scheme on more general three-dimensional settings, we bend the cylinder domain in Case III (see Fig.~\ref{fig:domain_sketch}), as is shown in Fig.~\ref{fig:bend_domain_sketch}. The same initial and boundary conditions as Case III are used. The numerical results of the electromagnetic field for $\lambda = 0$ and $1$ is displayed in Fig.~\ref{fig:3d_vec_field_E_B_bend}. It can be observed the qualitatively similar phenomena as Case~III (see Fig.~\ref{fig:3d_vec_field_E_B_ue}), that is, the electromagnetic field in the bulk of the domain develops much faster at small $\lambda$.}
\begin{figure}
    \centering
    \begin{minipage}{0.3\textwidth}
    \vspace{-2cm}
    ...
    \end{minipage}
    \hspace{1cm}
    \includegraphics[scale=0.2]{paper_version/primal_mesh_bend.png}
    \hspace{1cm}
