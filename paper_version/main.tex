...
\newcommand{\todo}[1]{\textcolor{green!50!black}{\fbox{\textbf{TODO: #1}}}}
...
\newcommand{\review}[1]{{\color{blue}{\if\ifUpdate1\sf{#1}\fi}}}
...
% \todo{Do not directly insert numbers of formulas, figure, sections. Always use \LaTeX's
%  \symbol{92}label-\symbol{92}ref facility.}
...
length, see Sec.~\ref{sec:rescaling} for the underlying scaling arguments and
...
semi-implicit time-stepping and base our illustration on the model situations shown in
Fig.~\ref{fig:domain_geometry}. Yet, we would like to remark that the mesh-based numerical
scheme proposed in this paper can in principle be adapted to settings more general
than those of Fig.~\ref{fig:domain_geometry}. \review{In particular, we do not assume any
  rotational symmetry, which would allow reduction to two spatial dimensions~\cite{bonotto_2022}.}
...
poses a challenge for simulations in settings encompassing different regimes.  Beside the
quasi-neutral limit, we remind that singularly perturbed problems occur in extensively
many physical models, e.g., in the case of vanishing viscosity problem~\cite{Kato_1984,
  bianchini_2005, temam_1997}, the zero-relaxation-limit of kinetic-type equations
~\cite{raymond_2009, degond_1996}, and the incompressible limit of compressible flows
~\cite{klainerman_1981}. Throughout, it is essential that numerical schemes remain valid
even if crucial model parameters approach the limit. These schemes are then said to be
\emph{asymptotic-preserving} (AP): Let us assume that we discretize a
parameter (denoted by $\lambda$) dependent model $P^\lambda$, which converges to a limit $P^0$ as
$\lambda \rightarrow 0$, by the scheme $P^\lambda_h$ where $h$ denotes some discretization
parameter, e.g., the mesh size. The AP property amounts to uniform convergence of
$P^\lambda_h$ to the $P^\lambda$ as $\lambda\rightarrow0$.  The concept is effectively
depicted by a commuting diagram in~\cite[][Fig.~1]{jin_2010}.  \review{AP schemes have
  been extensively studied for various problems, e.g., the diffusive limit of kinetic
  equations~\cite{jin_1999, liu_2023}, the low Mach-number limit of compressible flow
  models~\cite{degond_2007, haack_2010}, magnetohydrodynamics~\cite{chen_2023}, the
  quasi-neutral limit of drift-diffusion equations~\cite{chatard_2014, claire_2007}.  For
  more information readers are referred to the comprehensive reviews~\cite{jin_2010,
    degond_2017}.}

The content of this paper is organized as follows: In Sec.~\ref{sec:plasma-model}, we
...
reformulation of Maxwell's equations in the non-conducting region is presented. Sec.
...
full AP discretization in Sec.~\ref{sec:full-discretization} and do numerical
experiments in Sec.~\ref{sec:numerical_experiment}.
...
\begin{figure}
    \begin{subfigure}{0.45\textwidth}
        ...
        \draw[violet] (SS2) arc (101.31:180+101.31:1.5);
        ...
        \end{tikzpicture}
        \caption{Cylinder geometry}\label{fig:cylinder_geo}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \begin{tikzpicture}[scale=0.8]
            % Electrode 1
            \coordinate (O1) at (0,0);
            \draw[violet] (O1) rectangle ++(2,2) node[midway, blue] {Electrode};
            \fill[blue, opacity=0.2] (O1) rectangle ++(2,2);
            
            % Electrode 2
            \coordinate (O2) at (4.5,1);
            \draw[violet] (O2) rectangle ++(2,2) node[midway, red] {Electrode};
            \fill[blue, opacity=0.2] (O2) rectangle ++(2,2);
            
            % Connecting lines
            \coordinate (N1) at  ([shift={(0,2)}]O1);
            \coordinate (E1) at  ([shift={(2,0)}]O1);
            \coordinate (NE1) at ([shift={(2,2)}]O1);
            \coordinate (N2) at  ([shift={(0,2)}]O2); 
            \coordinate (E2) at  ([shift={(2,0)}]O2);
            \coordinate (NE2) at ([shift={(2,2)}]O2); 
            \draw[violet] (O1) -- (O2);
            \draw[violet] (N1) -- (N2);
            \draw[violet] (E1) -- (E2);
            \draw[violet] (NE1) -- (NE2);
             ...
             % Plasma
            \node at (barycentric cs:NE1=1,O2=1) {Plasma}; 
            \fill[red, opacity=0.2] (O2) rectangle ++(2,2);

            \coordinate (_O1) at (-0.5, -0.5);
            \coordinate (_O2) at (4.0,0.5);
            \draw[very thick] (_O1) rectangle ++(3,3);
            \draw[very thick] ([shift={(3,0)}]_O2) -- ++(0,3) -- ++(-3,0);
            \draw[violet] (_O2) -- ++(3,0);
            \draw[violet] (_O2) -- ++(0,3); 
            \coordinate (_N1) at  ([shift={(0,3)}]_O1);
            \coordinate (_E1) at  ([shift={(3,0)}]_O1);
            \coordinate (_NE1) at ([shift={(3,3)}]_O1);
            \coordinate (_N2) at  ([shift={(0,3)}]_O2); 
            \coordinate (_E2) at  ([shift={(3,0)}]_O2);
            \coordinate (_NE2) at ([shift={(3,3)}]_O2); 
            \draw[violet] (_O1) -- (_O2);
            \draw[very thick] (_N1) -- (_N2);
            \draw[very thick] (_E1) -- (_E2);
            \draw[very thick] (_NE1) -- (_NE2);
            
            \draw[-to] (barycentric cs:_N1=1,_NE1=1,_N2=1,_NE2=1) to[out=90,in=0] ([shift={(-0.5,0.5)}]barycentric cs:_N1=1,_NE1=1,_N2=1,_NE2=1) node[left, gray] {Insulator};
        \end{tikzpicture}
        \caption{Cuboid geometry}\label{fig:cube_geo}
    \end{subfigure}
    \caption{The special geometric setting suggested by the simulation of electric arcs,
        for which we discuss the new method.
        % \todo{Move geometry from Fig.~9 here. Display both side-by-side.}
    }
...
\begin{align} \label{equ:euler_}
    ...
    \end{pmatrix}.
\end{align}
The subscript $*\in \{e, i, \ldots\}$ labels electrons, ions or other species. The
...
\begin{equation} \label{equ:collision}
    \mathbf{R}_{e}^{\text{coll}} = \frac{\pi Ze^4m_e^{1/2}}{{(4\pi\epsilon)}^2{(k_B T_e)}^{3/2}}\text{\emph{ln}}\Lambda n_e n_i(\mathbf{u}_i - \mathbf{u}_e) = - \mathbf{R}_{i}^{\text{coll}},  
\end{equation}
...
\begin{align*} 
  ...
  \varepsilon_*^2 &:= \frac{m_*}{m_i} \text{ the species mass \emph{w.r.t.} the ion mass},\ \  * \in \{i,e,\ldots\}, &\\
...
\begin{subequations}\label{equ:euler-maxwell-rescaled}\label{resc}
\begin{align}
...
\review{
  \begin{remark}
    Structure preservation, that is, respecting inherent constraints satisfied by solution
    fields on the discrete level, is a major concern in discretization of
    (\ref{equ:euler-maxwell-rescaled}) or related reduced models. Important constraints
    are the positivity of densities, entropy inequalities, and zero divergence of the
    $\mathbf{B}$-field. Two main approaches can be distinguished:

    \begin{enumerate}[label={(\roman{*})}]
    \item The first is to combine all equations into one hyperbolic system of conservation
      laws and apply finite volume methods. Such numerical methods are rather simple to
      implement and also computationally efficient. The challenge is to ensure the
      stability and the divergence constraint, which requires suitable numerical fluxes
      and limiters. Schemes that are provably positive, entropy-stable, or divergence-free
      have been constructed, see~\cite{wu_2018, wu_2023a, wu_2023b, kumar_2011,
        fuchs_2009}, and the references therein.
    \item The second class of numerical methods tackles the electromagnetic and fluid
      subsystems separately. The rationale behind such a splitting strategy is that rather
      mature schemes for these parts are available. In particular, the divergence
      constraint can be easily handled by finite element exterior calculus. Yet the
      implementation of coupled solvers is usually complicated. To name some related work,~\cite{fuchs_2009} 
      applies dedicated finite volume schemes to each subsystem;~\cite{hu_2017, hiptmair_2018} 
      treat the electromagnetic part via finite element
      exterior calculus.
    \end{enumerate}
    The scheme proposed in this work belongs to the second category and by construction
    satisfies the divergence-free constraint at the discrete level. Yet, our focus is on
    the AP property and not primarily on structure preservation.
  \end{remark}
  % It is worth mentioning that an important aspect of devising numerical schemes is
  % the structure-preserving property which is concerned with keeping some constraints at
  % the discrete level. In this context, some typical constraints are the positivity of
  % densities and pressures, the entropy inequality, zero divergence of the
  % $\mathbf{B}$-field, etc. There are in general two ways of numerically approaching the
  % coupled system (\ref{equ:euler-maxwell-rescaled}) or its reduced models, e.g. the
  % magnetohydrodynamic (MHD) equations. The first approach is to lump all the equations
  % into one hyperbolic system of conservation laws and apply finite volume methods. Such
  % numerical methods are rather simple to implement and also computationally
  % efficient. Complications typically go to ensuring the stability and the divergence
  % constraint which requires suitable numerical fluxes and limiters. Schemes that are
  % provably positive, entropy-stable, or divergence-free have been constructed, see
  %~\cite{wu_2018, wu_2023a, wu_2023b, kumar_2011, fuchs_2009} and references therein. The
  % second class of numerical methods handles the electromagnetic part and the fluid part
  % separately. The rationale behind such a splitting strategy is that dedicated solvers for
  % subsystems are rather mature. Particularly, the divergence constraint can be easily
  % handled by finite element exterior calculus. Yet the implementation of coupled solvers
  % is usually complicated. To name some related work,~\cite{fuchs_2009} applies dedicated
  % finite volume schemes to each subsystem;~\cite{hu_2017, hiptmair_2018} treat the
  % electromagnetic part via finite element exterior calculus. The scheme proposed in this
  % work lies in the second category and by construction satisfies the divergence-free
  % constraint at the discrete level. Nevertheless, we do not pursue the
  % structure-preserving property while focusing on the asymptotic-preserving property in
  % this work.
}
...
\begin{minipage}{0.3\textwidth}
  \begin{subequations}\label{limit}
\begin{align}
...
\begin{remark}
 ...
 ~\cite[Chapt.~2]{remi_2014}~\cite{Peng_2008} for the quasi-neutral limit of the
  Maxwell-Euler system and~\cite{mark_1995, Eckhaus_1980} for some general theory.
\end{remark} 

\subsection{Boundary Conditions} \label{sec:BC}

We consider a simply-connected domain $\Omega$ comprising two subdomains $\Omega_P$
...
become clear through numerical experiments, see Sec.~\ref{sec:numerical_experiment}.
...
Hodge operator~\cite{hip_1999, bossavit1999}), as will be shown in Sec.~\ref{sec:fit}.
...
\begin{figure}
    \begin{subfigure}[v]{\textwidth}
        \centering
        \includegraphics[scale=0.3]{./figures/primal_mesh.png}
        \hspace{1cm}
        \includegraphics[scale=0.3]{./figures/dual_mesh.png}
        \caption{Cylinder geometry
          % \todo{Move primal-dual mesh pair from Fig.~9 here.}
        }\label{fig:primal_dual_meshes}
    \end{subfigure}
    \begin{subfigure}[v]{\textwidth}
        \centering
        \includegraphics[scale=0.25]{./figures/cube_mesh_primal.png}
        \hspace{1.2cm}
        \includegraphics[scale=0.25]{./figures/cube_mesh_dual.png}
        \caption{Cuboid geometry
          % \todo{Move primal-dual mesh pair from Fig.~9 here.}
        }\label{fig:primal_dual_meshes_cube}
    \end{subfigure}
    \caption{Examples of primal (left) and dual meshes (right)}
    \label{fig:examples_primal_dual_mehes}
\end{figure}
...
primal mesh $\mathcal{M}$. As is mentioned in Sec.~\ref{sec:BC}, the zero-magnetic-flux
...
a scalar value $\varphi_k$. Therefore, the boundary conditions (see Sec.~\ref{sec:BC})
...
In Sec.~\ref{sec:reform_continuous}, we have proposed a stabilization of the Maxwell
...
flux, and the Engquist-Osher flux, see~\cite[][Sec.~12.6]{leveque_2002}. We end up with the
semi-discretized equation:
\begin{equation}
  \dot{\overline{\mathbf{U}}}_k + \frac{1}{|V_k|}\sum_{l:\Tilde{A}_l\in\partial \Tilde{V}_k} |\Tilde{A}_l| \mathbf{f}^\text{num}\left(\overline{\mathbf{U}}_k, \overline{\mathbf{U}}_{\hat{k}(k,l)}, \mathbf{n}_l\right) = \overline{\mathbf{RHS}}_k,\ \  k = 1,2,\ldots,N_{\Tilde{V}}^P.
    \label{equ:fvm}
\end{equation}
...
\end{itemize}
We adopt the principle of least squares fitting and reconstruct a \emph{constant} vector
...
where
\begin{equation*}
    \mathbb{K} = [A_1\mathbf{n}_1, A_2\mathbf{n}_2, \ldots, A_{N_A}\mathbf{n}_{N_A}]^T,\ \ \ \mathbf{f} = [f_1, f_2, \ldots, f_{N_A}]^T,
\end{equation*}
...
The boundary conditions for the Euler system (see Sec.~\ref{sec:BC}) are implemented by
...
\end{align}
where $\ast \in \{e, i, \dots\}$ and $\bm{f}^\text{mass}$ denotes the mass flux which is defined in Sec.~\ref{sec:discrete_euler}. By $[\ldots]_k$ we mean taking the $k$-th component of both sides of the equation. We have the following boundary conditions:
    \begin{align}
...
$z$-axis. The idea is sketched in Fig.~\ref{fig:1d_reduction}.
...
3D FIT/DEC-FVM framework presented in Sec.~\ref{sec:spatial_discretization} and the
...
\review{
\begin{remark}
  When applying the ``projection to one dimension'' described in Sec.~\ref{sec:projection_to_1d}, the fully
  discrete 3D AP scheme~(\ref{equ:maxwell_faraday_ap})-(\ref{equ:Q_coll_ap}) collapses to the scheme of~\cite{degond_2012}
  presented in Sec.~\ref{sec:1d_fully_discrete_model}. This is not a coincidence, because recovering that scheme under
  dimensional reduction has guided our construction.
\end{remark}
}
...
presented in Sec.~\ref{sec:BC}. The primal-dual mesh pair
$(\mathcal{M}, \Tilde{\mathcal{M}})$ is generated as explained in
~\cite[][Chapt.~1]{fuchs_2021}. Examples are shown in Fig.~\ref{fig:examples_primal_dual_mehes}. 
We consider a two-fluid plasma model, which comprises electrons and ions. We set the mass
...
\end{align*}
which is nothing but the 1D model in Sec.~\ref{sec:projection_to_1d} after further
dropping $u_x, B_y, E_x$. We compare the results produced by the 1D scheme (see Sec.
~\ref{sec:1d_fully_discrete_model}) and those produced by the 3D scheme (see Sec.
...
\begin{figure}
    \centering
    \includegraphics[scale=1.2]{./figures/z_axis_reduction.eps}
    \caption{Case I, $\lambda = 1$: Comparison of 1D two-fluid model and of 3D results (at
      ...
      into account. \review{The 3D fields are computed for the axi-symmetric geometry
        shown in Fig.~\ref{fig:domain_sketch} and then restricted to the $z$-axis. They
        slightly differ from 1D results, possibly due to the impact of the cut-off boundary of the 3D domain.}}\label{fig:z_axis_reduction}
\end{figure}

\review{
  \subsection{Case II, convergence: 3D ``cuboid domain''}
    
  We rely on the geometry from Fig.~\ref{fig:cube_geo} and the setting in Fig.~\ref{fig:domain_sketch}. The computational
  domain is equipped with structured Cartesian meshes in Fig.~\ref{fig:primal_dual_meshes_cube}. The initial conditions are
\begin{equation*}
    n_\ast(\mathbf{x}, 0) = 1, \ \ \  p_\ast(\mathbf{x},0) = 1, \ \ \forall \mathbf{x}\in\Omega, \ast \in \{e, i\},
\end{equation*}
and zero for the other variables. We set
$\varphi(\mathbf{x}, t)|_{\Gamma_C^-} = 0, \varphi(\mathbf{x}, t)|_{\Gamma_C^+} = 10\times
t$, that is, one electrode is grounded and a ramp voltage is applied on the other
electrode. The results are computed for final time $t = 0.01$.  The scheme is tested for
different mesh resolutions and the convergence rates are
estimated. Tab.~\ref{tab:grid_study_cube} lists the errors for the $\mathbf{E}$-field,
$\mathbf{B}$-field, and electron densities with reference solutions generated on a
sufficiently fine mesh with $2^{18} = 262144$ primal cells. The results confirm uniform
algebraic convergence over $0\leq \lambda \leq 1$, a vindication of the AP property. It is
natural that the convergence rate does not exceed $1$, since the reconstruction of the
electromagnetic field discussed in Sec.~\ref{sec:recon_lorentz_f} is only of first
order and the numerical flux used for solving the Euler equations is also of first
order. The $1/2$-order reduction observed for the $L^2$-error of the $\mathbf{E}$-field
and the electron density is possibly the consequence of singularities of the solution due
to abutting different types of boundary conditions (electrodes and insulating boundary).
\begin{table}[!ht]
    \review{
    \centering
    \subfloat[\colorbox{yellow!30}{$\lambda = 1$}]{
        \begin{tabular}{c | c c | c c | c c }
            \hline\hline
            \multirow{2}{1em}{$N_V$} & \multicolumn{2}{c|}{$\mathbf{E}$-field}
            &
            \multicolumn{2}{c|}{$\mathbf{B}$-field} & \multicolumn{2}{c}{electron density} \\
            \cline{2-4} \cline{5-7}
            & $L^1$-error & $L^2$-error & $L^1$-error & $L^2$-error
            & $L^1$-error & $L^2$-error  \\
            \hline
             64   &  0.369 & 0.0712 & 5.88 & 1.14 & 6.67 & 1.83 \\
             512  &  0.195 & 0.0493 & 3.13 & 0.77 & 3.09 & 1.36 \\
             4096 &  0.087 & 0.0332 & 0.98 & 0.34 & 1.26 & 0.76 \\
             \hline
             aver. rate & 1.05 & 0.55 & 1.34 & 0.90 & 1.21 & 0.65 \\
             \hline\hline
        \end{tabular}
    }\\
    \vspace{0.5cm}\subfloat[\colorbox{yellow!30}{$\lambda = 0.01$}]{
        \begin{tabular}{c | c c | c c | c c }
            \hline\hline
            \multirow{2}{1em}{$N_V$} & \multicolumn{2}{c|}{$\mathbf{E}$-field}
            &
            ...
            & $L^1$-error & $L^2$-error  \\
            \hline
             64   &  0.141 & 0.0271 & 1.52E-3 & 2.67E-4 & 6.67 & 1.83 \\
             512  &  0.094 & 0.0214 & 8.21E-4 & 1.47E-4 & 3.09 & 1.36 \\
             4096 &  0.041 & 0.0113 & 3.84E-4 & 6.93E-5 & 1.26 & 0.76 \\
             \hline
             aver. rate & 0.93 & 0.66 & 1.00 & 0.98 & 1.21 & 0.65 \\
             \hline\hline
        \end{tabular}
    }\\
    \vspace{0.5cm}\subfloat[\colorbox{yellow!30}{$\lambda = 0$}]{
        \begin{tabular}{c | c c | c c | c c }
            \hline\hline
            \multirow{2}{1em}{$N_V$} & \multicolumn{2}{c|}{$\mathbf{E}$-field}
            &
            ...
            & $L^1$-error & $L^2$-error  \\
            \hline
             64   &  0.127 & 0.0274 & 9.46E-4 & 1.58E-4 & 6.67 & 1.83 \\
             512  &  0.100 & 0.0212 & 6.26E-4 & 1.05E-4 & 3.09 & 1.36 \\
             4096 &  0.038 & 0.0110 & 2.49E-4 & 4.33E-5 & 1.26 & 0.76 \\
             \hline
             aver. rate & 0.96 & 0.69 & 1.00 & 0.98 & 1.21 & 0.65 \\
             \hline\hline
        \end{tabular}
    }
    \caption{\review{Case II:  $L^1$ and $L^2$ errors of the $\mathbf{E}$-field, the
        $\mathbf{B}$-field, and the electron density and their convergence rates for
        different values of $\lambda$. The reference solution is computed at a sufficient mesh
        resolution with $2^{18} = 262144$ primal cells.}
    }
    \label{tab:grid_study_cube}}
\end{table}
}
...
We stick to the setting in Fig.~\ref{fig:domain_sketch} and the mesh in Fig.~\ref{fig:primal_dual_meshes} using the following initial conditions
\begin{equation*}
    n_\ast(\mathbf{x}, 0) = 1, \ \ \  p_\ast(\mathbf{x},0) = 1, \ \ \forall \mathbf{x}\in\Omega, \ast \in \{e, i\}.
\end{equation*}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \draw[blue, -to] (0,0) -- (0.7,0) node[below]{\small $y$};
        \end{tikzpicture}
        \includegraphics[scale=0.2]{./figures/E-field_lambda-1.png}\hspace{0.6cm}
        \includegraphics[scale=0.2]{./figures/B-field_lambda-1.png}
        \includegraphics[scale=0.2]{./figures/electronVel_lambda-1.png}
    ...
    \end{subfigure}
     \begin{subfigure}[b]{\textwidth}
        ...
        \draw[blue, -to] (0,0) -- (0.7,0) node[below]{\small $y$};
        \end{tikzpicture}
        \includegraphics[scale=0.2]{./figures/E-field_lambda-1e-1.png}
        \hspace{0.3cm}
        \includegraphics[scale=0.2]{./figures/B-field_lambda-1e-1.png}
        \includegraphics[scale=0.2]{./figures/electronVel_lambda-1e-1.png}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \draw[blue, -to] (0,0) -- (0.7,0) node[below]{\small $y$};
        \end{tikzpicture}
        \includegraphics[scale=0.2]{./figures/E-field_lambda-0.png}
        \hspace{0.3cm}
        \includegraphics[scale=0.2]{./figures/B-field_lambda-0.png}
        \includegraphics[scale=0.2]{./figures/electronVel_lambda-0.png}
        \caption{\colorbox{yellow!30}{$\lambda = 0.0$}}
    \end{subfigure}
    \caption{Case III: Snapshots of the $\mathbf{E}$-field, $\mathbf{B}$-field, and $\mathbf{u}_e$ for different values of $\lambda$ at $t = 0.5$. } 
...
consistent with the fact that $\lambda$ characterizes the timescale of the
...
\begin{figure}
  \centering
    \includegraphics[scale=0.9]{./figures/data-vs-time_stepVoltage }
...
The stabilization proposed in Sec.~\ref{sec:reform-discrete} is essential to achieve
...
\begin{minipage}[t]{0.4\textwidth}\centering
  \includegraphics[width=\textwidth]{./figures/stabilizationComparision_stepVoltage}
...
\end{minipage}%    
 \begin{minipage}[t]{0.6\textwidth}\centering
   \includegraphics[width=\textwidth]{./figures/data-vs-z_stepVoltage}
...
\review{
  \subsection{Case IV: 3D curved cylindrical domain}
  
  To demonstrate the performance of the proposed scheme in more general three-dimensional
  settings, we bend the cylinder domain in Case III (see Fig.~\ref{fig:domain_sketch}), as
  is shown in Fig.~\ref{fig:bend_domain_sketch}. The same initial and boundary conditions
  as in Case III are imposed. Primal and dual meshes with straight edges and flat faces
  are generated by extrusion in the direction of a bent central axis. 

  The numerical approximations of the electromagnetic fields for $\lambda = 0$ and $1$ are
  displayed in Fig.~\ref{fig:3d_vec_field_E_B_bend}. We observe qualitatively similar
  phenomena to Case~III (see Fig.~\ref{fig:3d_vec_field_E_B_ue}), that is, the
  electromagnetic field in the bulk of the domain develops much faster at small
  $\lambda$.}
\begin{figure}
    \centering
    \begin{minipage}{0.3\textwidth}
    \vspace{-2cm}
        \resizebox{\textwidth}{!}{
            \begin{tikzpicture}
                ...
                \fill[blue, opacity=0.2] (O1) circle (1);
                \draw[very thick] (O1) circle (1.5);
                \coordinate (O2) at (4.5,2);
                %\draw[violet] (O2) circle (1);
                \node at (O2) {\textcolor{red}{Electrode}};
                
                \coordinate (S1) at ([shift={(-0.707,0.707)}]O1);
                \coordinate (S2) at ([shift={(-0.196,0.98)}]O2);
                \coordinate (P1) at ([shift={(0.707,-0.707)}]O1);
                \coordinate (P2) at ([shift={(0.196,-0.98)}]O2);
                \draw[violet] (S1) to[out=45, in=180 + 10] (S2);
                \draw[violet] (P1) to[out=45, in=180 + 10] (P2);
                \coordinate (SS1) at ([shift={({-0.707*1.5},{0.707*1.5})}]O1);
                \coordinate (SS2) at ([shift={({-0.196*1.5},{0.99*1.5})}]O2);
                \coordinate (PP1) at ([shift={({0.707*1.5},{-0.707*1.5})}]O1);
                \coordinate (PP2) at ([shift={({0.196*1.5},{-0.99*1.5})}]O2);
                \draw[very thick] (SS1) to[out=45, in=180 + 10] (SS2);
                \draw[very thick] (PP1) to[out=45, in=180 + 10] (PP2); 
                \draw[very thick] (SS2) to[out=-180+10, in=180+10] (PP2);
                \draw[very thick] (PP2) to[out=10, in=10] (SS2);
                \draw[violet] (S2) to[out=-180+10, in=180+10] (P2) to[out=10, in=10] (S2);
                \fill[red, opacity=0.2] (S2) to[out=-180+10, in=180+10] (P2) to[out=10, in=10] (S2);
                \draw[-to] ([shift={(3,1.5)}]SS1) to[out=90,in=0] ([shift={(-0.5,0.7)}]barycentric cs:SS1=1,SS2=1) node[left, gray] {Insulator};
                \node at (barycentric cs:P1=1,SS2=1) {Plasma};
            \end{tikzpicture}
        }
    \end{minipage}
    \hspace{1cm}
    \includegraphics[scale=0.2]{./figures/primal_mesh_bend.png}
    \hspace{1cm}
    \includegraphics[scale=0.2]{./figures/dual_mesh_bend.png}
    \caption{\review{Case IV: geometry (left); primal mesh (middle); dual mesh (right).}}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale=0.3]{./figures/E-field_lamnbda-1_bend.png}\hspace{1cm}
        \includegraphics[scale=0.3]{./figures/B-field_lamnbda-1_bend.png}
        \caption{\colorbox{yellow!30}{$\lambda = 1$}}
    \end{subfigure}
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[scale=0.3]{./figures/E-field_lamnbda-0_bend.png}\hspace{1cm}
        \includegraphics[scale=0.3]{./figures/B-field_lamnbda-0_bend.png}
...
% \todo{Carefully check references!}
